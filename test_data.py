...
class RandomDataGenerator():
	...
	def create_line_data(self, v1, v2, n=100, randmod=1):
		d = []
		for i in range(0, n):
			...
			d.append({'UniqueID': str(random.choice([1, 2, 3, 4, 5])), 'TrackID': str(random.choice([1, 2, 3])), 'Timestamp': time.time(), 'Geopos': str([x, y]), 'Accdata': str([random.random() * 20 for i in range(0, 3)])})
		return d		
