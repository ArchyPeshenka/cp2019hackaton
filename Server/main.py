...
class ServerInterfaceApp(App):
	...
	def get_values(self):
		if self.mainscreen.ids["'userid'"].text != '':
			userid = str(self.mainscreen.ids["'userid'"].text)
		else:
			userid = False

		if self.mainscreen.ids["'trackid'"].text != '':
			trackid = str(self.mainscreen.ids["'trackid'"].text)
		else:
			trackid = False

		if self.mainscreen.ids["'email'"].text != '':
			email = str(self.mainscreen.ids["'email'"].text)
		else:
			email = False
	...
	def generate_map_and_open(self):
		...
		print('!!!!', userid, trackid)
		self.DataWorker.load_data()
		#checking by userid:
		if userid:
			self.DataWorker.sort_by_users(None, userid)

		if trackid:
			self.DataWorker.sort_by_track(None, trackid)
			
		self.DataWorker.prod_get_graph(self.p_value, 'map.html', radius=0.001, k=self.k_value)
